...
pub fn countRows(rows: *std.ArrayList(Row), row:* Row, i: usize) void {
    ...
    for (rows.items[min..max]) |*copy| {
        ...
        std.log.debug("COPY => {s} ((( {d} >>> {d}\n", .{copy.card, copy.*.copies, copy.matches});
    }

    for (rows.items[min..max], i+1..) |*copy, idx| {
        countRows(rows, copy, idx);
    }
}
